...
async def run_worker():
    ...
    if isinstance(queue_manager, MemoryQueueManager):
        print(
            "❌ MemoryQueueManager does not support clustering. Use a different queue manager for distributed processing."
        )
        sys.exit(1)
    ...
    async with queue_manager:
        ...
        async with request_queue, response_queue:
            ...
            print(
                f"✅ Worker subscribed to requests with subscription ID: {subscription.id}"
            )
    ...
    async with queue_manager:
        ...
        async with request_queue, response_queue:
            ...
            worker_match_subscriber = WorkerMatchSubscriber(
                response_subscriber, response_queue.get_worker_id()
            )
            response_subscription = await response_queue.subscribe(
                worker_match_subscriber, True
            )
            print(
                f"✅ Broker subscribed to responses with subscription ID: {response_subscription.id}"
            )
            ...
            try:
                ...
                while True:
                    ...
                    print(
                        f"📤 Broker request #{request_count}: Calculate factorial of {number}"
                    )
