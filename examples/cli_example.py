...
class PrintSubscriber(Subscriber[MyMessage]):
    ...
    async def on_event(
        self, event: QueueEvent[MyMessage], event_queue: EventQueue[MyMessage]
    ) -> None:
        """Print the received event details."""
        print(
            f"📨 Received event #{event.id} at {event.created_at.strftime('%H:%M:%S')}"
        )
        ...
        async with queue:
            ...
            try:
                ...
                while True:
                    ...
                    message = MyMessage(
                        msg=f"Hello from eventy! Message #{message_count} at {current_time}"
                    )
                    event = await queue.publish(message)

                    print(
                        f"📤 Published message #{message_count} (Event ID: {event.id})"
                    )
