...
class AbstractFileEventQueue(EventQueue[T], ABC):
    ...
    async def publish(self, payload: T) -> QueueEvent[T]:
        ...
        while True:
            ...
            try:
                ...
                _LOGGER.debug(
                    f"Event file {event_file} already exists, recalculating next_event_id"
                )
                self._initialize_event_counter()
                continue
    ...
    async def search_results(
        ...
        async for result in self._iter_results(
            event_id__eq, worker_id__eq, created_at__gte, created_at__lte
        ):
    ...
    async def _notify_subscribers_cached(self, event: QueueEvent[T]):
        ...
        if subscriber_count > 0:
            _LOGGER.info(
                f"Notified {subscriber_count} subscribers about event {event.id}"
            )
    ...
    async def search_claims(
        ...
        async for claim in self._iter_claims(
            worker_id__eq, created_at__gte, created_at__lte
        ):
    ...
    async def count_claims(
        ...
        count = 0
        async for _ in self._iter_claims(
            worker_id__eq, created_at__gte, created_at__lte
        ):
            count += 1
        return count
