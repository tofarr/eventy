...
class TestMemoryEventQueueComprehensive(AbstractEventQueueCase):
    ...
    def get_event_queue(self) -> EventQueue[MockPayload]:
        ...
        return MemoryEventQueue(event_type=MockPayload)
    ...
    async def test_memory_queue_serialization_deserialization(self):
        ...
        original_payload = MockPayload("test message", 123)
    ...
    async def test_memory_queue_event_reconstruction(self):
        ...
        payloads = [MockPayload(f"msg{i}", i) for i in range(3)]
        published_events = []
    ...
    async def test_memory_queue_thread_safety_simulation(self):
        ...
        async def publish_events():
            for i in range(10):
                payload = MockPayload(f"concurrent_msg{i}", i)
                await queue.publish(payload)
        ...
        await asyncio.gather(subscribe_all(), publish_events(), search_operations())
    ...
    async def test_memory_queue_error_handling_in_serialization(self):
        ...
        class FailingSerializer(Serializer[MockPayload]):
            """Serializer that always fails"""

            def serialize(self, obj: MockPayload) -> bytes:
                raise ValueError("Serialization failed")

            def deserialize(self, data: bytes) -> MockPayload:
                raise ValueError("Deserialization failed")

        # Create queue with failing serializer
        queue = MemoryEventQueue(event_type=MockPayload, serializer=FailingSerializer())
    ...
    async def test_memory_queue_custom_serializer(self):
        ...
        queue = MemoryEventQueue(event_type=MockPayload, serializer=JsonSerializer())

        payload = MockPayload("json test", 456)
