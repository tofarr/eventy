...
class PollingFileEventQueue(AbstractFileEventQueue[T]):
    ...
    async def __aenter__(self):
        ...
        self._subscription_polling_task = asyncio.create_task(
            self._subscription_polling_loop()
        )

        _LOGGER.info(
            f"Started polling file event queue at {self.root_dir} with interval {self.polling_interval}s"
        )
        return self
    ...
    async def __aexit__(self, exc_type, exc_value, traceback):
        ...
        if (
            self._subscription_polling_task
            and not self._subscription_polling_task.done()
        ):
    ...
    async def _poll_for_events(self):
        ...
        for event_file in event_files:
            try:
                ...
                _LOGGER.error(
                    f"Error processing event file {event_file}: {e}", exc_info=True
                )
        ...
        try:
            while not self._stop_polling:
                try:
                    ...
                    _LOGGER.error(
                        f"Error during subscription polling: {e}", exc_info=True
                    )
            ...
            _LOGGER.error(
                f"Unexpected error in subscription polling loop: {e}", exc_info=True
            )
        finally:
        ...
        for subscription_file in self.subscriptions_dir.iterdir():
            ...
            try:
                ...
                if current_mtime > last_mtime:
                    _LOGGER.debug(
                        f"Detected change in subscription file: {subscription_file.name}"
                    )
                ...
                _LOGGER.warning(
                    f"Error checking subscription file {subscription_file}: {e}"
                )
