...
class TestPollingFileEventQueue(AbstractEventQueueTestBase):
    ...
    async def test_polling_queue_isolation_by_directory(self):
        ...
        try:
            queue1 = PollingFileEventQueue(
                root_dir=self.root_path, payload_type=str, polling_interval=0.1
            )
            queue2 = PollingFileEventQueue(
                root_dir=root_path2, payload_type=str, polling_interval=0.1
            )
    ...
    async def test_polling_queue_background_tasks(self):
        ...
        if hasattr(queue, "_polling_task") and queue._polling_task:
            self.assertTrue(
                queue._polling_task.done() or queue._polling_task.cancelled()
            )
        if (
            hasattr(queue, "_subscription_polling_task")
            and queue._subscription_polling_task
        ):
            self.assertTrue(
                queue._subscription_polling_task.done()
                or queue._subscription_polling_task.cancelled()
            )
