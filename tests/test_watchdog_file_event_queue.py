...
class TestWatchdogFileEventQueue(AbstractEventQueueTestBase):
    ...
    async def test_watchdog_queue_isolation_by_directory(self):
        ...
        try:
            queue1 = WatchdogFileEventQueue(
                root_dir=self.root_path,
                payload_type=str
            )
            queue2 = WatchdogFileEventQueue(
                root_dir=root_path2,
                payload_type=str
            )
    ...
    async def test_watchdog_queue_observer_lifecycle(self):
        ...
        self.assertFalse(hasattr(queue, '_observer') and queue._observer and queue._observer.is_alive())
    ...
    async def test_watchdog_queue_event_handlers(self):
        ...
        async with self.queue:
            ...
            if hasattr(self.queue, '_subscription_handler') and self.queue._subscription_handler:
