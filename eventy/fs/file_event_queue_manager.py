    ...
    def __post_init__(self):
        ...
        if self._use_watchdog is None:
            try:
                ...
                _LOGGER.info("Watchdog library detected - will use WatchdogFileEventQueue")
            except ImportError:
                self._use_watchdog = False
                _LOGGER.info("Watchdog library not available - will use PollingFileEventQueue")
    ...
    async def __aexit__(self, exc_type, exc_value, traceback):
        ...
        for payload_type, queue in self._queues.items():
            try:
                ...
                _LOGGER.warning(f"Error closing queue for {payload_type}: {e}", exc_info=True)
