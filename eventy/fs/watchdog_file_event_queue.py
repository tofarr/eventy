    ...
    def on_created(self, event):
        ...
        if not event.is_directory:
            ...
            try:
                ...
                loop.call_soon_threadsafe(self._pending_events.put_nowait, event.src_path)
            except RuntimeError:
                # If no event loop is running, log a warning
                _LOGGER.warning(f"No event loop running when trying to queue event: {event.src_path}")
