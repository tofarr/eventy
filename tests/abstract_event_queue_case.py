...
class BasePayloadSubscriber(Subscriber[BaseTestPayload]):
    """Subscriber for base payload type"""
    
    def __init__(self):
        self.payload_type = BaseTestPayload
        self.received_events = []
        
    async def on_event(self, event: QueueEvent[BaseTestPayload]) -> None:
        self.received_events.append(event)
...
class AbstractEventQueueCase(ABC):
    ...
    async def queue(self) -> EventQueue[TestPayload]:
        """Get a fresh EventQueue instance for testing"""
        return self.get_event_queue()
    
    @pytest.fixture
    def payload(self) -> TestPayload:
        """Create a test payload"""
        return TestPayload(message="test message", value=42)
    ...
    @pytest.mark.asyncio
    async def test_publish_single_subscriber_receives_event(self, queue, payload, subscriber):
    ...
    @pytest.mark.asyncio
    async def test_publish_multiple_subscribers_all_receive_events(self, queue, payload):
        ...
        payload1 = TestPayload("message1", 1)
        payload2 = TestPayload("message2", 2)
    ...
    @pytest.mark.asyncio
    async def test_publish_with_subscriber_error_continues_processing(self, queue, payload):
    ...
    @pytest.mark.asyncio
    async def test_unsubscribe_existing_subscriber_returns_true(self, queue, subscriber):
    ...
    @pytest.mark.asyncio
    async def test_unsubscribed_subscriber_no_longer_receives_events(self, queue, payload):
        ...
        await queue.publish(TestPayload("message1", 1))
        ...
        await queue.publish(TestPayload("message2", 2))
    ...
    async def test_search_events_multiple_events(self, queue):
        ...
        payloads = [TestPayload(f"message{i}", i) for i in range(5)]
        published_events = []
    ...
    async def test_search_events_with_date_filters(self, queue):
        ...
        page = await queue.search_events(
            created_at__min=past,
            created_at__max=future
        )
        ...
        page = await queue.search_events(
            created_at__min=future,
            created_at__max=future + timedelta(hours=1)
        )
    ...
    async def test_search_events_pagination_multiple_pages(self, queue):
        ...
        payloads = [TestPayload(f"message{i}", i) for i in range(7)]
    ...
    async def test_search_events_pagination_edge_cases(self, queue):
        ...
        payloads = [TestPayload(f"message{i}", i) for i in range(3)]
    ...
    async def test_get_all_events_mixed_existing_nonexistent(self, queue, payload):
        ...
        events = await queue.get_all_events([published_event.id, 999999, published_event.id])
    ...
    async def test_concurrent_publishing(self, queue):
        ...
        async def publish_events(start_index: int, count: int):
            for i in range(count):
                payload = TestPayload(f"message{start_index + i}", start_index + i)
                await queue.publish(payload)
    ...
    async def test_concurrent_subscribe_unsubscribe(self, queue):
        ...
        await asyncio.gather(*[
            subscribe_and_unsubscribe(subscriber) 
            for subscriber in subscribers
        ])
    ...
    async def test_large_payload_handling(self, queue):
        ...
        large_payload = TestPayload(large_message, 42)
    ...
    async def test_queue_state_consistency_after_errors(self, queue, payload):
        ...
        for i in range(3):
            test_payload = TestPayload(f"message{i}", i)
